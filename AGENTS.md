...
当前 runtime 主要基于 `@mariozechner/pi-coding-agent` 的 session/tool 能力。产品侧可以选择不同 harness/backend：默认稳定路径是 Pi；Ousia 是显式选择后才启用的独立 framework companion；Codex、Hermes、OpenClaw 都是 Pie 侧 adapter / managed runtime 接入，其中 OpenClaw 和 Hermes 会拉起本地 gateway/service，启动资源成本明显高于 Pi/Ousia session。
...
- Hermes 可由 Pie 代管本地 gateway service，但仍按早期接入处理。
- OpenClaw 可由 Pie 代管本地 gateway service，是真实接入，不再只是架构预留；但资源成本高，桌面自动恢复时要受启动预算和资源余量约束。
...
- Hermes/OpenClaw gateway 启停属于 harness managed service，当前内聚在 `src/agents/harness-services/hermes.ts` 和 `src/agents/harness-services/openclaw.ts`。
...
- `src/runtime/main.ts`：Pie 客户端 runtime 编排入口，初始化 profile home，并按 `AgentHarnessDefinition` 启动当前可用 channel、framework companion 和 managed harness service。
- `src/runtime/environment.ts`：Runtime Environment 抽象，负责解析/创建工作目录并表达生命周期状态；当前不是安全沙盒。
- `src/agents/harness-registry.ts`：harness/backend 集中注册入口；每个 harness 同时声明 `AgentHarnessAdapter`、`AgentFrameworkRuntime`、可选 managed harness service 和 skill sources。
...
- `src/core/agent-harness.ts`：`AgentFrameworkRuntime` 类型和 runtime companion 解析；harness 注册入口不要放在这里，统一放在 `src/agents/harness-registry.ts`。
...
- 多 harness 抽象已经有轻量 registry，但 Codex/Hermes/OpenClaw 仍属早期接入。不要在这些 backend 稳定前继续扩大统一 Agent API。
