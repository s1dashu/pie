...
export function loadConfig(argv: string[] = process.argv.slice(2)): FeishuBotConfig {
	...
	const { path: assistantSystemPromptPath, content: assistantSystemPrompt } = resolveAssistantSystemPrompt(env);
	...
	return {
		...
		outputToolCallsToIm: parseBooleanFlag(env.FEISHU_BOT_IM_TOOL_CALLS, false),
